...
import { Check, Clock, ChevronDown, ChevronUp, ShoppingBag, Gift, Sparkles, ChefHat, Bell } from "lucide-react"
...
export default function OrderConfirmation() {
  ...
  const { items, subtotal, serviceFee, total, orderType, clearCart } = useCart()
  ...
  const { status, isReady, isPreparing, connected } = useOrderTracking(orderId, {
    onReady: useCallback(() => {
      setOrderReady(true)
      ...
      <div className="w-full max-w-2xl space-y-8 py-12">
        ...
        <div className="flex flex-col items-center justify-center text-center space-y-8">
          ...
          <div className="relative">
            <div
              className={`w-28 h-28 md:w-40 md:h-40 bg-matcha rounded-full flex items-center justify-center shadow-premium transform transition-all duration-700 ease-in-out ${pulse ? "scale-100 shadow-2xl" : "scale-105 shadow-xl"
                }`}
            >
          ...
          <div className="space-y-3">
            ...
            <p className="text-lg md:text-xl text-espresso-light font-sans max-w-md mx-auto leading-relaxed">
              ...
              <br />
              <span className="text-sm font-bold opacity-60 mt-2 block">Screen will reset in {countdown}s</span>
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-2">
            {['pending', 'preparing', 'ready'].map((s, idx) => (
              <div key={s} className="flex items-center">
                <div className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-500 ${status === s
                    ? s === 'ready'
                ...
                {idx < 2 && (
                  <div className={`w-8 h-1 mx-1 rounded ${(status === 'preparing' && idx === 0) ||
                      (status === 'ready' && idx <= 1)
                      ? 'bg-matcha'
                      : 'bg-gray-200'
                    }`} />
          ...
          <div className="bg-white px-8 py-6 md:px-12 md:py-8 rounded-[2rem] shadow-float border border-cream-dark/30 transform hover:-translate-y-1 transition-transform duration-300">
            <p className="text-xs md:text-sm font-bold text-espresso uppercase tracking-widest mb-2 md:mb-3 opacity-60">Order Number</p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <div className="flex items-center gap-4 bg-white px-6 py-4 rounded-2xl shadow-sm border border-cream-dark/50">
              ...
              <div className="text-left">
                <p className="text-[10px] font-bold text-espresso uppercase tracking-widest opacity-60">Estimated Wait</p>
                <p className="text-lg font-bold text-espresso">~{estimatedWait} mins</p>
              </div>
            </div>
            {savedOrderType && (
              <div className="flex items-center gap-4 bg-white px-6 py-4 rounded-2xl shadow-sm border border-cream-dark/50">
                ...
                <div className="text-left">
                  <p className="text-[10px] font-bold text-espresso uppercase tracking-widest opacity-60">Order Type</p>
                  <p className="text-lg font-bold text-espresso capitalize">{savedOrderType.replace('-', ' ')}</p>
                </div>
              </div>
            )}
          </div>
                ...
                <div className="space-y-2 mt-4">
                  ...
                  <div className="flex items-center justify-center gap-1">
                    {Array.from({ length: 10 }).map((_, i) => (
                      <div
                        key={i}
                        className={`w-7 h-7 rounded-full flex items-center justify-center text-xs ${i < (currentStamps % 10)
                  ...
                  <p className="text-center text-sm text-espresso-light">
                    ...
                    {currentStamps >= 10 && (
                      <span className="ml-2 text-gold-dark font-bold">
                        🎉 {Math.floor(currentStamps / 10)} free drink{Math.floor(currentStamps / 10) > 1 ? 's' : ''} available!
                      </span>
                    )}
                  </p>
                </div>
        ...
        <div className="space-y-4 pt-4">
          <Link href="/" className="block transform hover:scale-[1.02] transition-transform">
            <Button
              className="w-full h-20 text-2xl font-bold bg-matcha text-white hover:bg-matcha/90 border-2 border-transparent hover:border-matcha/20 rounded-[2rem] shadow-premium"
            >
              Start New Order
            </Button>
          </Link>
