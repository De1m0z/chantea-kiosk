...
import { Mic, MicOff, Loader2, Check, X, ShoppingCart, RotateCcw, AlertCircle, AudioLines } from 'lucide-react'
...
export function VoiceOrderModal({ isOpen, onClose, products }: VoiceOrderModalProps) {
    const {
        state,
        transcript,
        interimTranscript,
    ...
    const findProduct = useCallback((item: VoiceOrderItem): {
        product: Product
        size: ProductSize
        modifiers: Modifier[]
    } | null => {
        ...
        for (const item of result.items) {
            ...
            for (const mod of modifiers) {
                ...
                if (groupNameLower.includes('sugar')) {
                    sugarModifier = mod
                } else if (groupNameLower === 'ice level' || groupNameLower === 'ice_level' || groupNameLower === 'ice') {
                    iceModifier = mod
            ...
            if (sugarModifier) {
                ...
                if (name.includes('0') || name.includes('no ') || name.includes('zero')) sugarLevel = 0
            ...
            const modifierTotal = modifiers.reduce((sum, m) => sum + Number(m.price_adjustment || 0), 0)
                ...
                <div className="bg-espresso rounded-t-[2.5rem] px-8 pt-8 pb-6 text-center relative overflow-hidden">
                    ...
                    <div className="relative">
                        <div className="flex items-center justify-center gap-3 mb-1">
                            ...
                            <h2 className="text-2xl font-serif font-black text-white">Voice Order</h2>
                        </div>
                        <p className="text-white/60 text-sm font-medium">Tell us what you'd like</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col items-center gap-6 p-8">
                    ...
                    {state === 'idle' && isSupported && (
                        <>
                            <p className="text-center text-espresso-light text-sm px-4 leading-relaxed">
                                ...
                                For example: <span className="font-semibold text-espresso">"Two taro milk teas, medium, with less sugar and boba"</span>
                            </p>
                            <button
                            ...
                            >
                                <div className="absolute inset-0 rounded-full bg-matcha/20 animate-ping" style={{ animationDuration: '2s' }} />
                                <Mic className="w-12 h-12 relative z-10" />
                            </button>
                            <p className="text-sm text-espresso-light font-medium">Tap to start speaking</p>
                        </>
                    )}
                    ...
                    {state === 'listening' && (
                        <>
                            ...
                            <p className="text-sm font-bold text-red-500 animate-pulse">Listening... Tap to finish</p>
                            ...
                            <div className="w-full bg-white rounded-2xl p-5 min-h-[60px] shadow-sm border border-gray-100">
                                <p className="text-sm text-espresso leading-relaxed">
                                    ...
                                    {interimTranscript && (
                                        <span className="text-espresso-light italic">{interimTranscript}</span>
                                    )}
                                    {!transcript && !interimTranscript && (
                                        <span className="text-espresso-light italic">Say your order...</span>
                                    )}
                                </p>
                            </div>
                        </>
                    ...
                    {state === 'processing' && (
                        <>
                            ...
                            <p className="text-sm text-espresso-light font-medium">Understanding your order...</p>
                            {transcript && (
                                <div className="w-full bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                                    <p className="text-sm text-espresso-light italic">"{transcript}"</p>
                                </div>
                            )}
                        </>
                    )}
                    ...
                    {state === 'error' && (
                        <>
                            ...
                            <p className="text-sm text-red-600 text-center px-4 font-medium">{error}</p>
                            <button
                                onClick={reset}
                    ...
                    {state === 'done' && result && (
                        <>
                            ...
                            <div className="w-full bg-matcha/5 border border-matcha/20 rounded-2xl p-5">
                                <p className="text-sm font-semibold text-espresso">{result.message}</p>
                            </div>
                            ...
                            {result.items.length > 0 && (
                                <div className="w-full space-y-3">
                                    ...
                                    {result.items.map((item, idx) => {
                                        const found = findProduct(item)
                                        return (
                                            <div key={idx} className="flex items-center gap-4 bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                                                {/* Product image */}
                                                ...
                                                <div className="flex items-center gap-1.5 flex-shrink-0">
                                                    ...
                                                    <span className="text-sm font-bold text-espresso">{formatPrice(item.unit_price * item.quantity)}</span>
                                                </div>
                                            </div>
                                        )
                                    })}
                            ...
                            {result.not_found.length > 0 && (
                                <div className="w-full space-y-2">
                                    ...
                                    {result.not_found.map((item, idx) => (
                                        <div key={idx} className="flex items-center gap-3 bg-red-50 rounded-2xl px-4 py-3 border border-red-100">
