...
export async function updateProductModifiers(
  ...
  const res = await fetchApi<ApiResponse<Product>>(
...
export async function getModifierGroup(id: number): Promise<ModifierGroup> {
  const res = await fetchApi<ApiResponse<ModifierGroup>>(
    `/modifier-groups/${id}`,
  );
  return res.data;
}
...
export async function updateOrderItemStatus(
  ...
  const res = await fetchApi<ApiResponse<Order>>(
...
export async function customerLogin(
  ...
  const res = await fetchApi<
    ApiResponse<{ token: string; customer: Customer }>
  >("/customer/login", {
    method: "POST",
...
export async function getDailySales(): Promise<DailySalesSummary> {
  const res = await fetchApi<ApiResponse<DailySalesSummary>>(
 ...
 */
export async function lookupCustomerByUsername(
  username: string,
): Promise<LoyaltyCustomer> {
  const res = await fetchApi<ApiResponse<LoyaltyCustomer>>(
    `/loyalty/lookup?username=${encodeURIComponent(username)}`,
  );
  return res.data;
}
 ...
 */
export async function previewStamps(
  items: Array<{ product_id: number; quantity: number }>,
): Promise<{ stamps_to_earn: number }> {
  const res = await fetchApi<ApiResponse<{ stamps_to_earn: number }>>(
    "/loyalty/preview",
    {
      method: "POST",
 ...
 * Redeem free drink
 */
export async function redeemFreeDrink(
  customerId: number,
): Promise<{ redeemed: boolean; remaining_stamps: number; message: string }> {
  const res = await fetchApi<
    ApiResponse<{
      redeemed: boolean;
...
export async function processVoiceOrder(
  ...
  const res = await fetchApi<ApiResponse<VoiceOrderResult>>("/voice-order", {
    method: "POST",
